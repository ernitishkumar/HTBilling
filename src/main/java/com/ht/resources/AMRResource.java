...
public class AMRResource {
	...
	public Response uploadAMRFile(@FormDataParam("file") InputStream uploadedInputStream,
		...
		// System.out.println("AMR File upload started with file name as:
		...
		if (uploadedInputStream != null && fileDetail != null) {
			...
			if (savedPMRFilePath != null) {
				...
				if (amrReadingToInsert != null) {
					readingAlreadyInserted = AMRReadingDAO.getByMeterNoAndReadingDate(amrReadingToInsert.getMeterNo(),
							amrReadingToInsert.getReadingDate());
	...
	public Response removeReading(@PathParam("id") int id) {
		...
		if (removedReading != null) {
			return Response.status(Status.OK).entity(removedReading).build();
		} else {
			return Response.status(Status.EXPECTATION_FAILED).entity(new ErrorBean("Unable to delete reading."))
					.build();
		}
	}
	...
	public Response approve(AMRReading amrReading) {
		...
		plant = PlantsDAO.getByMainMeterNo(amrReading.getMeterNo());
		if (meterDetails != null && plant != null) {
			...
			if (meterReading.getActiveEnergy().compareTo(previousReading.getActiveEnergy()) != -1
					...
					&& meterReading.getActiveTodThree().compareTo(previousReading.getActiveTodThree()) != -1
					&& !isReadingAlreadyAdded) {
		...
		if (updatedAMRReading != null) {
			return Response.status(Status.OK).entity(updatedAMRReading).build();
		} else {
			ErrorBean error = new ErrorBean();
			if (meterDetails == null) {
				error.setErrorMessage("Meter not Present. Please Add the Meter Frist.");
			}else if (plant == null) {
				error.setErrorMessage("Meter not Installed on Any Plant. Please Check.");
			} else if (isReadingAlreadyAdded) {
				error.setErrorMessage(
						"Reading is already present or Previous Month Reading is not Present. Please Check! Unable to insert meter Reading.");
