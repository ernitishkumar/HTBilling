...
public class ReadingResource {
	...
	public Response saveReading(MeterReading meterReading) {
		System.out.println("Inserting Reading for : "
				+ meterReading.getMeterno() + " with mf : "
				+ meterReading.getMf());
		...
		if (insertedReading != null) {
			return Response.status(Status.CREATED).entity(insertedReading)
					.build();
		} else {
	...
	public ArrayList<ViewMeterReadings> getAll() {
		...
		for (Plant p : plants) {
			...
			MeterReading currentMonthReading = meterReadingsDAO
					.getCurrentMonthMeterReadings(meterNo);
			if (currentMonthReading != null
				...
				if (currentMonthReading.getSrfrFlag() == 1) {
					...
					currentMonthReading = meterReadingsDAO
							.getCurrentMonthMeterReadings(checkMeterNo);
					viewMeterReadings.setPreviousMeterReading(meterReadingsDAO
							.getPreviousMonthMeterReadings(checkMeterNo));
					viewMeterReadings
							.setCurrentMeterReading(currentMonthReading);
				} else {
					viewMeterReadings.setPreviousMeterReading(meterReadingsDAO
							.getPreviousMonthMeterReadings(meterNo));
					viewMeterReadings
							.setCurrentMeterReading(currentMonthReading);
				}
				if (currentMonthReading != null
	...
	@Produces(MediaType.APPLICATION_JSON)
	public ArrayList<ViewMeterReadings> getByCircle(
			@PathParam("circleName") String circle) {
		System.out.println("GetReadings by circle started for circle : "
				+ circle);
		ArrayList<ViewMeterReadings> viewReadings = new ArrayList<ViewMeterReadings>();
		if (circle != null) {
	...
	@Produces(MediaType.APPLICATION_JSON)
	public ArrayList<ViewMeterReadings> getByDeveloperUsername(
	...
	@Produces(MediaType.APPLICATION_JSON)
	public ViewMeterReadings getByMainMeterno(
			@PathParam("meterno") String meterno) {
		System.out.println("GetReadings by meterno started for meterno : "
				+ meterno);
		...
		if (meterno != null) {
			...
			viewMeterReadings.setDeveloper(developersDAO.getById(plant
					.getDeveloperId()));
			MeterReading currentMonthReading = meterReadingsDAO
					.getCurrentMonthMeterReadings(meterNo);
			if (currentMonthReading.getDiscardedFlag() == 0) {
				...
				if (currentMonthReading != null
						&& currentMonthReading.getDeveloperValidation() == 1) {
					viewMeterReadings.setConsumption(consumptionDAO
							.getBifercationFlagByPlantIdAndDate(plant.getId(),
									currentDate));
				}
				viewMeterReadings = viewReading;
			}
		}
		return viewReading;
	}
	...
	@Path("/validate")
	@Produces(MediaType.APPLICATION_JSON)
	public Response validateReadings(@QueryParam("role") String role,
			@QueryParam("currentReadingId") int currentReadingId,
			@QueryParam("previousReadingId") int previousReadingId) {
		System.out.println("validateReadings called for role " + role
				+ " with pre id: " + previousReadingId + " and curr id: "
				+ currentReadingId);
		boolean validated = false;
		if (role != null) {
			if (role.equalsIgnoreCase("htcell")
					|| role.equalsIgnoreCase("admin")) {
				validated = meterReadingsDAO.updateHTCellValidation(
						currentReadingId, 1);
			} else if (role.equalsIgnoreCase("circle")) {
				validated = meterReadingsDAO.updateCircleCellValidation(
						currentReadingId, 1);
			} else if (role.equalsIgnoreCase("developer")) {
				MeterReading currentReading = meterReadingsDAO
						.getById(currentReadingId);
				MeterReading previousReading = meterReadingsDAO
						.getById(previousReadingId);
				if (previousReading != null && currentReading != null) {
					System.out
							.println("Validating Reading since both current and previous readings is present");
		...
		if (role != null && validated) {
			...
			return Response.status(Status.OK)
					.entity(new MessageBean("validated")).build();
		} else {
			return Response.status(Status.EXPECTATION_FAILED)
					.entity(new ErrorBean("not validated")).build();
		}
	...
	@Path("/discard")
	@Produces(MediaType.APPLICATION_JSON)
	public Response discardReadings(UserRoles role,
			@QueryParam("readingId") int readingId) {
		boolean discarded = false;
		if (role != null) {
			System.out
					.println("Discarding readings by role: " + role.getRole());
			if (role.getRole().equalsIgnoreCase("htcell")
					|| role.getRole().equalsIgnoreCase("admin")) {
				System.out.println("Going to discard readings ");
				discarded = meterReadingsDAO.updateDiscardedFlagByAdmin(
						readingId, 1, role);
			} else if (role.getRole().equalsIgnoreCase("developer")) {
				System.out.println("inside for discard from developer");
				discarded = meterReadingsDAO.updateDiscardedFlagByDeveloper(
						readingId, 1, role);
			}
			/*
			 * else if(role.getRole().equalsIgnoreCase("developer")){
		...
		if (discarded) {
			return Response.status(Status.OK)
					.entity(new MessageBean("discarded")).build();
		} else {
			return Response.status(Status.EXPECTATION_FAILED)
					.entity(new ErrorBean("not discarded")).build();
		}
	...
	public Response saveSRFRReading(SRFRReadings srfrReading) {
		...
		meterReading.setReactiveQuadrantOne(srfrReading
				.getReactiveQuadrantOne());
		meterReading.setReactiveQuadrantTwo(srfrReading
				.getReactiveQuadrantTwo());
		meterReading.setReactiveQuadrantThree(srfrReading
				.getReactiveQuadrantThree());
		...
		preCheckReading.setActiveTodThree(srfrReading
				.getPreCheckActiveTodThree());
		preCheckReading.setReactiveQuadrantOne(srfrReading
				.getPreCheckReactiveQuadrantOne());
		preCheckReading.setReactiveQuadrantTwo(srfrReading
				.getPreCheckReactiveQuadrantTwo());
		preCheckReading.setReactiveQuadrantThree(srfrReading
				.getPreCheckReactiveQuadrantThree());
		...
		currCheckReading.setActiveTodThree(srfrReading
				.getCurCheckActiveTodThree());
		currCheckReading.setReactiveQuadrantOne(srfrReading
				.getCurCheckReactiveQuadrantOne());
		currCheckReading.setReactiveQuadrantTwo(srfrReading
				.getCurCheckReactiveQuadrantTwo());
		currCheckReading.setReactiveQuadrantThree(srfrReading
				.getCurCheckReactiveQuadrantThree());
