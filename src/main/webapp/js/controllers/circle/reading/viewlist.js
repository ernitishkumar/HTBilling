angular.module("htBillingApp").controller('ListCircleReadingsController', ['$http', '$scope', '$location', '$routeParams','authService','utilService','localStorageService', function ($http, $scope, $location, $routeParams, authService,utilService,localStorageService) {
	...
	function loadReadings() {
		$scope.loading = true;
		console.log("getting data for Circle "+$scope.userRole);
		$http(
				{
					method: 'GET',
					url: 'backend/readings/month/'+$scope.formData.billMonth,
					params: {
						circle: $scope.userRole.circle
						}
				}
		).then(
				function (response) {
