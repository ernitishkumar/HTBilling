angular.module("htBillingApp").controller('SRFRMeterReadingController', ['$http', '$scope', '$location', 'authService',function ($http, $scope, $location, authService) {
				...
				url: 'backend/plants/meterno/'+this.formData.meterno
					...
					function(response) {
						...
						if (status === 200) {
							...
							$scope.formData.checkMeterNo = meterData.plant.checkMeterNo;
							$scope.plainmeter = false;
	...
	this.processForm = function () {
			...
			this.formData.checkMeterNo = $scope.formData.checkMeterNo;
			$http({
				method: 'POST',
				url: 'backend/readings/srfr',
				headers:{
